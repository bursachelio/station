...
class Main
  ...
  def create_train
    ...
    raise 'Тип поезда может быть только "pass" или "cargo"!' unless %w[pass cargo].include?(type)
    ...
    train = case type
      when 'pass'
        PassengerTrain.new(num, manufacturer)
      when 'cargo'
        CargoTrain.new(num, manufacturer)
      end
  
    @trains << train
    puts "Поезд #{train.num} типа #{train.type} с количеством вагонов #{train.vagons_count}" \
         " от производителя #{train.manufacturer} создан"
    train
  rescue StandardError => e
    puts "Ошибка: #{e.message}"
    retry
